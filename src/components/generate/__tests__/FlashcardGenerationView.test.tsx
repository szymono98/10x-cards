    ...
    >
      {proposals.map((p: { front: string; back: string }, i: number) => (
        <div key={i} data-testid={`flashcard-${i}`}>
          ...
          <button onClick={() => onEdit(i, "edited front", "edited back")}>
            Edit
          </button>
        </div>
      ))}
...
describe("FlashcardGenerationView", () => {
  ...
  describe("Save Handlers", () => {
    it("should save accepted flashcards", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Flashcards.*saved successfully/i)
        ).toBeInTheDocument();
      });
    });
  ...
  describe("React Optimization Features", () => {
    ...
    it("should defer non-urgent updates during save with useTransition", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Flashcards are saved successfully")
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe("Functional Features", () => {
    ...
    it("should save all flashcards", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/All flashcards.*saved successfully/i)
        ).toBeInTheDocument();
      });
    });
